...
class SongshuhuiSpider(scrapy.Spider):
    name='songshuhuiSpider'
    allowed_domains=['songshuhui.net']

    start_urls=['http://songshuhui.net/?s=%E7%9C%9F%E7%9B%B8']

    # start_urls=['http://www.douban.com/note/427790805']

    root_class='甩开假新闻'
    #一级分类下面的频道
    default_channel='真实新闻'
     #源网站的名称
    sourceSiteName='松鼠科学会'

    default_tag='真实新闻'

    page_url_pattern=re.compile(r'^http://songshuhui.net/archives/\d+')
    non_page_url_pattern=re.compile(r'http://songshuhui.net/')
    ...
    def dealWithNonPage(self,response,url):
        xpath_str='div[@id="content"]/div[@class="step bspacing10'
