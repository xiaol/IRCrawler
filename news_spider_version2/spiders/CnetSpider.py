...
class CnetSpider(scrapy.Spider):
    ...
    previous_page_pat=re.compile(r'<a\s*?target="_self" href="(.*?)".*?>\\u4e0b\\u4e00\\u9875</a>')
    nonpage_url_pat=re.compile(r'<a\s*?href="(http://www\.cnetnews\.com\.cn.*?)" target="_blank">')
    ...
    def extractTitle(self,response):
        raw_title_str=response.xpath('//div[@class="qu_leftd"]').extract()[0]
    ...
    def extractTime(self,response):
        ...
        raw_time_str=response.xpath('//p[@class="qu_zuc"]').extract()
    ...
    def extractContent(self,response):
        rawContent=response.xpath('//div[@class="qu_content_div"]').extract()[0]
    ...
    def extractImgUrl(self,response):
        rawContent=response.xpath('//div[@class="qu_content_div"]').extract()
        if not len(rawContent):
            return None
    ...
    def extractTag(self,response):
        tag=response.xpath('//p[@class="qu_zuc"]/a/text()').extract()
        print "tag,%s"%tag
        return tag
    ...
    def dealWtihNonPage(self,response,url):
        ...
        pages_arr=response.xpath('//ul[@class="qu_guanzh"]').extract()[0]  #/li[@class="box masonry-brick"
...
# http://t.qianzhan.com//dazahui/p-2.html
    ...
    def getPrevoiuPageUrl(self,response):
        title_sign=u'下一页'
        xpath_str='//div[@class="pagebar"]'.decode('utf8')
        ...
        if searchResult:
            ...
            if page_url_str:
                page_url_str="http://www.cnetnews.com.cn"+page_url_str
