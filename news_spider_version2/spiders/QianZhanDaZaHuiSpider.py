...
class QianZhanSpider(scrapy.Spider):
    ...
    allowed_domains=['t.qianzhan.com']
    ...
    nonpage_url_pat=re.compile(r'<a href="(http://t\.qianzhan\.com.*?)">')
    ...
    def extractTitle(self,response):
        raw_title_str=response.xpath('//div[@class="article-title"]').extract()[0]
    ...
    def extractTime(self,response):
        raw_time_str=response.xpath('//div[@class="author-time"]/p').extract()[0]
    ...
    def extractContent(self,response):
        rawContent=response.xpath('//div[@class="article-details"]').extract()
    ...
    def extractTag(self,response):
        tag=response.xpath('//div[@class="article-tags-share"]/div[@class="tags"]/a/text()').extract()
        print "tag,%s"%tag
        return tag
    ...
    def dealWtihNonPage(self,response,url):
        ...
        pages_arr=response.xpath('//div[@class="articles"]/ul/li').extract()  #/li[@class="box masonry-brick"
