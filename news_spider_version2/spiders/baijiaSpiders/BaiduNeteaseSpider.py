...
class BaiduNeteaseSpider(scrapy.Spider):
    name='baidu.com.netease'
    ...
    def __init__(self, url=None, topic=None, *args, **kwargs):
        ...
        super(BaiduNeteaseSpider, self).__init__(*args, **kwargs)
        # keywords = []
        self.start_urls=[]
        topic = topic.split('s')
        ...
        self.start_urls.append('https://www.baidu.com/s?ie=utf-8&f=8&rsv_bp=1&rsv_idx=2&tn=baiduhome_pg&wd=site:163.com++%s'%self.keyword)
        # for keyword in keywords:
        # self.start_urls.append('https://www.google.com.hk/?gws_rd=ssl#safe=strict&q=site:163.com++%s'%self.keyword)
        # self.start_urls.append('http://www.google.com.hk/?gws_rd=ssl')
        # self.start_urls.append('http://www.google.com.hk/search?q=site:163.com++%s&num=100&ie=utf-8&oe=utf-8&aq=t&rls=org.mozilla:en-US:official&client=firefox-a&channel=fflb'%self.keyword)
    ...
    def parse(self,response):
        ...
        news_block_items= sel.css('.result.c-container')[0:2]
        items=[]
        itemMetas=[]
        ...
        # news_block_items_ex
        for news_block_item in news_block_items:
            item=CommentItem()
            ...
            try:
                ...
                sourceUrl=news_block_item.xpath('./h3/a/@href').extract()[0]
            except IndexError:
                continue
            # sourceUrl="http://news.163.com/15/0506/02/AOT9E0AL00014AED.html"
            sourceUrl = self.DecryptSourceUrl(sourceUrl)
            item['_id']=sourceUrl
            ...
            try:
                ...
                title=news_block_item.xpath('./h3/a/descendant-or-self::text()').extract()
            except IndexError:
                continue
            titleStr=''.join(title)
    ...
    def DecryptSourceUrl(self, sourceUrl):
