...
class IfanrSpider(scrapy.Spider):
    ...
    def extractTitle(self,response):
        raw_title_str=response.xpath('//div[@class="entry-header"]').extract()[0]
    ...
    def extractTime(self,response):
        raw_time_str=response.xpath('//div[@class="main main-news"]').extract()
    ...
    def extractContent(self,response):
        rawContent=response.xpath('//div[@class="entry-content"]').extract()[0]
    ...
    def extractImgUrl(self,response):
        rawContent=response.xpath('//div[@class="entry-content"]').extract()
        if not len(rawContent):
            return None
    ...
    def extractTag(self,response):
        tag=response.xpath('//div[@class="entry-meta-tags"]/a/text()').extract()
        print "tag,%s"%tag
        return tag
    ...
    def dealWtihNonPage(self,response,url):
        ...
        pages_arr=response.xpath('//div[@class="main main-news"]/div/div').extract()  #/li[@class="box masonry-brick"
...
# http://t.qianzhan.com//dazahui/p-2.html
    ...
    def getPrevoiuPageUrl(self,response):
        title_sign=u'下一页'
        xpath_str='//div[@class="content-nav"]'.decode('utf8')
