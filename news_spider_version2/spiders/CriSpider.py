...
class CriSpider(scrapy.Spider):
    ...
    time_pat=re.compile(r'<span id="pubtime_baidu">([\d\-:\s]*?)</span>',re.DOTALL)
    ...
    def extractTitle(self,response):
        raw_title_str=response.xpath('//div[@class="news_ts_l"]').extract()[0]
    ...
    def extractTime(self,response):
        ...
        raw_time_str=response.xpath('//div[@class="news_ts_ly"]').extract()
    ...
    def dealWtihNonPage(self,response,url):
        ...
        pages_arr=response.xpath('//div[@class="news_ts_l"]').extract()  #/li[@class="box masonry-brick"
